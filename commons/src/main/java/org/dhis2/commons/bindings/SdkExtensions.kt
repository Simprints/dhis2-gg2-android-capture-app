...
fun D2.stockUseCase(programUid: String): StockUseCase? =
    useCaseModule()
        .stockUseCases()
        .withTransactions()
        .uid(programUid)
...
fun D2.dataSetInstanceSummaries(): List<DataSetInstanceSummary> = dataSetModule().dataSetInstanceSummaries().blockingGet()

fun D2.dataElement(uid: String) =
    dataElementModule()
        .dataElements()
        .uid(uid)
        .blockingGet()
...
fun D2.countTeiImportConflicts(teiUid: String): Int =
    importModule()
        .trackerImportConflicts()
        .byTrackedEntityInstanceUid()
        .eq(teiUid)
...
fun D2.enrollment(uid: String) =
    enrollmentModule()
        .enrollments()
        .uid(uid)
        .blockingGet()

fun D2.enrollmentInProgram(
    teiUid: String,
    programUid: String,
): Enrollment? =
    enrollmentModule()
...
fun D2.canCreateEventInEnrollment(
    enrollmentUid: String,
    stagesToHide: List<String>,
): Boolean =
    enrollmentModule()
        .enrollmentService()
...
fun D2.countEventImportConflicts(eventUid: String): Int =
    importModule()
        .trackerImportConflicts()
        .byEventUid()
        .eq(eventUid)
