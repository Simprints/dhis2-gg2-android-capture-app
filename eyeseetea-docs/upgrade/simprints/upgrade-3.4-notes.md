...
### B6. `FormViewModel.fieldListChannel` can silently drop the `finish=true` signal

- **Baseline file:** `form/src/main/java/org/dhis2/form/ui/FormViewModel.kt`
  (`fieldListChannel`, `processCalculatedItems()`, `displayResult()`)
- **Status:** `pending` — mitigated in the Simprints branch, root cause unfixed
- **Evidence:** `fieldListChannel` is `Channel<FieldListConfiguration>(capacity = 1,
  onBufferOverflow = BufferOverflow.DROP_OLDEST)`. `FieldListConfiguration.finish`
  is the signal that triggers `runDataIntegrityCheck()` — the check that decides
  whether to close the form and navigate away after a save. `DROP_OLDEST` treats
  a `finish = true` message exactly like an ordinary recompute request: if
  another message lands in the channel before the `finish = true` one is read,
  it is silently discarded, and nothing resends it — the save/close chain never
  runs, with no error and no retry path. Confirmed on device 2026-09-04 with
  `Timber` traces at every hop (see `upgrade-3.4-notes.md`'s "registerLast hang,
  second cause" write-up and `customization-files.md` §2.10 for the full trace).
- **Problem:** this is generic Oslo behavior, not Simprints-specific — any save
  flow that submits multiple `FormIntent`s in a tight burst (little to no
  user-paced typing in between to naturally space out the channel) can hit it.
  Simprints' `registerLast` reproduces it reliably because its own two-step save
  (biometric value + auto-triggered form save) is a code-driven burst of 4
  intents with no pacing, but an ordinary manual "press save" (`OnSave` + `OnFinish`,
  2 intents) is not structurally immune, just less likely to collide.
- **Fix to promote (needs prototyping, not done this session):** either (a)
  switch `onBufferOverflow` to the channel's default `SUSPEND` so no message is
  ever dropped, or (b) add priority logic so a pending `finish = true` message
  can never be overwritten by a later `finish = false` one. Both require load
  testing (fast typing in a long form) to confirm no perceptible recompute lag
  before trusting either in a shared, all-forks file.
- **Mitigation shipped in Simprints for now:** `EnrollmentActivity` debounces its
  `onSaveClick()` trigger by 500ms after `registerLast`'s `markAsPendingSave()`,
  giving the channel time to drain — see `customization-files.md` §2.10. Scoped
  to Simprints' own file, doesn't touch the shared channel, but doesn't fix the
  root cause either.
...
### registerLast hang, second cause: `fieldListChannel` race silently drops `finish=true` (found and fixed 2026-09-04)

Reported by the user, day of delivery: same symptom as the 2026-09-02 `registerLast` hang (biometric value saved, spinner never resolves, TEI never navigates) — but reproduced *after* that day's three fixes were already in place and verified. Confirmed via device log with `Timber` traces added temporarily at every hop between `EnrollmentActivity.markAsPendingSave()` and `FormView`'s `actionsChannel.collect` (removed after the fix; see `customization-files.md` §2.10 for the permanent documentation).

**Root cause**: `FormViewModel.kt`'s `fieldListChannel` (Oslo, untouched by Simprints) is `Channel<FieldListConfiguration>(capacity = 1, onBufferOverflow = BufferOverflow.DROP_OLDEST)` — a deliberate coalescing buffer so a burst of recompute requests doesn't queue up `composeList()` calls, keeping only the most recent one. It does not distinguish an ordinary recompute (`finish = false`) from the one that also triggers `runDataIntegrityCheck()` (`finish = true`, sent when the user finishes/saves). If a `finish = false` message lands in the channel after a `finish = true` one was sent but before it's been read, `DROP_OLDEST` silently discards the `finish = true` message — and nothing else ever resends it, so the save/close chain (`runDataIntegrityCheck` → `handleDataIntegrityResult` → `FormActions.OnFinish` → navigation) never runs. The user is left staring at a spinner with no error and no retry path.

`registerLast`'s own save sequence makes this collision far more likely than an ordinary manual save: `EnrollmentPresenterImpl.saveBiometricValue()` submits the biometric value's `OnTextChange`/`OnSave` intents, then (via `markAsPendingSave()`) `EnrollmentActivity`'s `onFieldItemsRendered` immediately fires `formView.onSaveClick()` → `saveDataEntry()`, which submits its own `OnSave`(last focused field)/`OnFinish` intents — four intents in a tight, code-driven burst (no user-paced typing in between) all competing for the same 1-slot channel.

**Why it was intermittent, and why "all mandatory fields collected in search" reproduces it reliably**: on Simprints Kenya's 2.43 test server (unlike the 2.42 production server), all mandatory attributes are configured as searchable, so the user can fill them all in the search screen and reach the enrollment form with nothing left to fill in — pressing save fires `registerLast`'s two-step save with zero other field-edit traffic in between to naturally space out the channel. When some mandatory fields are only reachable from the form itself, typing them first spaces out the channel's turnover by chance, lowering (not eliminating) the collision odds — confirmed by reproducing the hang once even after typing several fields first, just requiring more attempts.

**Fix**: `EnrollmentActivity`'s `onFieldItemsRendered` now debounces the `onSaveClick()` trigger by 500ms (`PENDING_SAVE_DEBOUNCE_MS`, a cancellable `lifecycleScope.launch { delay(...) }` `Job`, restarted on every subsequent `onFieldItemsRendered` while `pendingSave` is still true) instead of firing on the first signal. This gives the channel time to drain between the biometric-value save and the form save, the same way it does naturally when a user edits fields with pauses in between. Verified twice on device with the exact failing scenario (all mandatory attributes pre-filled from search, no form interaction before pressing save) — both the debounce delay itself and the full save→navigate chain observed correctly in the trace.

**Options considered and rejected for today:**
- Disable the save/"Next" action until the channel is idle — doesn't apply here since the failing save isn't user-triggered at all (it's `registerLast`'s automatic post-capture save); would also need to expose a "channel idle" signal from `FormViewModel` that doesn't exist today, and wouldn't remove the race, just relocate its window.
- Give priority to `finish = true` in the channel itself (never let a later `finish = false` message overwrite one still pending) — the structurally correct fix, but requires new coalescing logic in `FormViewModel.kt` (Oslo, shared by every fork and every form in the app), which is riskier to write and fully regression-test the same day as delivery than a debounce scoped to Simprints' own file.
- Switch `onBufferOverflow` to the channel's default `SUSPEND` (never drop, producers wait for the consumer instead) — a 1-line change, but still touches the shared Oslo channel used by every form save in the app; carries a real (if likely small) risk of a perceptible lag during fast typing bursts, since every recompute would now be processed instead of coalesced. Worth prototyping and load-testing (typing quickly in a long form) before trusting it, which there wasn't time for today.

Both rejected options remain valid candidates for `develop-eyeseetea` — see "Improvements to promote to `develop-eyeseetea`" below.

**Why 3.3.1 never hit this, confirmed by diffing against `develop-simprints`:** `fieldListChannel` itself (`capacity = 1`/`DROP_OLDEST`) is byte-for-byte identical in 3.3.1 — the race is not new to this upgrade. What's new is that the biometric-value save now uses that same channel at all. In 3.3.1, `saveBiometricValue()` called `biometricsUiModel!!.onTextChange(value)`/`.onSave(value)`, routed through `FieldUiModel.Callback` — reattached synchronously within the same Compose pass that painted the form, no async channel involved for that leg. The 2026-09-02 fix (see that day's write-up above) replaced those calls with `view.submitFormIntent(...)` specifically to stop that callback from going stale — and in doing so made the biometric-value save submit through `pendingIntents`/`fieldListChannel`, the same channel `registerLast`'s auto-triggered form save also uses. **Fixing the stale-callback bug is what turned the biometric save into a second source of traffic competing for a channel it never touched before.** The `pendingSave`/`onFieldItemsRendered`/`markAsPendingSave` mechanism itself is identical between the two versions (diffed `EnrollmentPresenterImpl.kt`/`EnrollmentActivity.kt` against `develop-simprints` to confirm), so this callback-to-intent change is the only structural difference that explains it.

**If this keeps recurring, stop tuning the debounce and redesign the save path instead.** The debounce works, but it's papering over two independent save triggers (biometric value + form finish) that got funneled into the same shared channel as a side effect of an unrelated fix, not because that's the right design. A future recurrence (needing to raise the delay, or a new variant of the hang) is a signal to look at a single explicit save call that carries both the biometric value and the finish signal together — removing the race by construction — rather than adding another timing-based workaround on top of this one.
