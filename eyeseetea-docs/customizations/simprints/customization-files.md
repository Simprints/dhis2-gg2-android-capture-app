...
- The attribute UID (`BP90qVFNazj`) currently only exists on the `simprints-dev` instance; it has to be created on every other instance before this ships to it. If the attribute is missing, enrollment still completes and the template is simply not saved.
