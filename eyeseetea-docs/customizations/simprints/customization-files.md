...
**Second, independent bug found 2026-09-04, same symptom (registerLast hangs after save)**: `EnrollmentActivity`'s `onFieldItemsRendered` callback used to call `formView.onSaveClick()` synchronously the moment `pendingSave` was true. `saveBiometricValue()` submits the biometric value's `OnTextChange`/`OnSave` intents, then (via `markAsPendingSave()`) that immediately-following `onSaveClick()` submits its own `OnSave`(last focused field)/`OnFinish` intents — four intents in a tight burst, all funneling into `FormViewModel`'s `fieldListChannel` (Oslo, `Channel<FieldListConfiguration>(capacity = 1, onBufferOverflow = BufferOverflow.DROP_OLDEST)`). That channel coalesces recompute requests by design (drop the oldest pending one when a newer one arrives), but doesn't distinguish an ordinary "recompute" message from the `finish = true` one that triggers `runDataIntegrityCheck()` — if a third message lands before the `finish = true` one is read, it silently replaces it, and the save/close chain never resumes. Confirmed on device with `Timber` traces added temporarily at every hop of the chain (`fieldListChannel.consumeEach`/`processCalculatedItems`/`displayResult`), removed after the fix. Reproduces reliably when the enrollment form's mandatory attributes are all collected in the search screen (Simprints Kenya's 2.43 test server, unlike the 2.42 production server, allows all mandatory attributes to be searchable) — the user reaches the form with nothing left to fill, so registerLast's own two-step save fires with no other field-edit traffic in between to naturally space it out. **Fix**: `EnrollmentActivity` now debounces the `onSaveClick()` trigger 500ms (`PENDING_SAVE_DEBOUNCE_MS`, restarted on every subsequent `onFieldItemsRendered` while `pendingSave` is still true, via a cancellable `lifecycleScope` coroutine `Job`) instead of firing on the first signal — giving the channel time to drain, the same way it naturally does when a user edits fields with pauses in between. Verified twice on device with the exact failing scenario (all mandatory attributes pre-filled from search, no form interaction before pressing save).

**Tech debt (deferred 2026-09-04):** the debounce is a timing-based mitigation, not a structural fix — the underlying race in Oslo's `fieldListChannel` (any `finish = true` message can be silently dropped by `DROP_OLDEST` if something else recomputes right after) is unfixed and file-agnostic to Simprints; it could in principle affect the plain "press save" flow too (also 2 intents in a burst), just with a narrower collision window since biometrics adds two extra intents on top. A structural fix (e.g. switching `onBufferOverflow` to the channel's default `SUSPEND` instead of `DROP_OLDEST`, or adding priority logic so a pending `finish = true` message is never overwritten) belongs in `develop-eyeseetea` since `FormViewModel.kt`'s channel is untouched Oslo code — deferred to after this upgrade's delivery to avoid changing shared, hard-to-fully-regression-test code under deadline pressure.

**Why 3.3.1 never needed this debounce**: `fieldListChannel` (Oslo, `capacity = 1`/`DROP_OLDEST`) is byte-for-byte identical in `develop-simprints` (3.3.1) — the race itself is not new. What changed is where the biometric value's save signal enters the pipeline. In 3.3.1, `saveBiometricValue()` called `biometricsUiModel!!.onTextChange(value)`/`.onSave(value)`, which route through `FieldUiModel.Callback` — a callback that, in 3.3.1's architecture, was reattached synchronously within the same Compose pass that painted the form (no async channel involved for that leg of the save). The 2026-09-02 fix (§2.10 above) changed this to `view.submitFormIntent(...)` specifically to bypass that callback going stale — which means the biometric-value save now also funnels through `pendingIntents`/`fieldListChannel`, the exact same channel the auto-triggered form save uses. **Fixing the stale-callback bug is what turned the biometric save into a second source of traffic on the shared channel it never competed with before.** Confirmed by diffing `develop-simprints`'s `EnrollmentPresenterImpl.kt`/`EnrollmentActivity.kt` against this branch: the `pendingSave`/`onFieldItemsRendered`/`markAsPendingSave` mechanism is identical in both versions, so this is the only structural difference that explains why the channel race went from theoretical to reproducible.

**If this keeps causing problems, the debounce is not the place to keep patching.** It masks a timing collision between two independent save paths that both got funneled into the same shared channel as an indirect side effect of an unrelated bug fix — not because the design calls for it. If the debounce value ever needs raising again, or a new variant of this hang shows up, that is a signal to stop tuning the delay and instead redesign how Simprints' `registerLast` save reaches the form (e.g. a single explicit save call carrying both the biometric value and the finish signal together, instead of two independent triggers racing through the same channel), rather than adding more timing-based workarounds on top.
...
- `eyeseetea-docs/scripts/simprints/replicate-biometrics-template.ts` (replicates the attribute metadata to another DHIS2 server — see technical note below)

DHIS2 metadata (not app code, tracked here per the placement-hierarchy rule):
- Tracked entity attribute `Biometrics Template` (UID `BP90qVFNazj`, `valueType: LONG_TEXT`), linked wherever the `Biometrics` (GUID) attribute `KdZcTAZfIk4` is linked: the `Person` TET and, on `simprints-dev`, the programs `0.0 General Registration` and `1.6 Child Health` as `programTrackedEntityAttribute`.
...
- The attribute UID (`BP90qVFNazj`) exists on `simprints-dev` and Ghana Play (imported 2026-09-24 with the script below, linked on `Person`, `0.0 General Registration` and `1.6 Child Health`); it has to be created on every other instance, production included, before this ships to it. If the attribute is missing, enrollment still completes and the template is simply not saved.
- Replicating the metadata to another instance: `npx tsx eyeseetea-docs/scripts/simprints/replicate-biometrics-template.ts --target <Name> --mode plan|validate|commit` (credentials from `local.properties`, `simprins<Name>Server|User|Pwd`). Only the attribute UID must match, because the app hardcodes it; program and TET UIDs are taken from the target instance. The script copies the attribute from `simprints-dev` and adds only the missing link to the target's own TETs and programs that carry `Biometrics`, so it does not overwrite the rest of their configuration (e.g. `0.0 General Registration` has different mandatory attributes on `simprints-dev` and Ghana Play). It backs up the target objects, validates before importing, and can be run again safely. Always run `plan` first and review the programs it leaves out: instances differ in which programs carry `Biometrics` (production has four), and the same program UID can mean a different program on another instance. Importing needs a superuser account.
