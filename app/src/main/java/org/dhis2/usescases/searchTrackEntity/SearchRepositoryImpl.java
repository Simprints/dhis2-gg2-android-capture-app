...
public class SearchRepositoryImpl implements SearchRepository {
    ...
    // EyeSeeTea customization - Biometric Verification Persistence
    // Needed by updateAttributeValue() to persist the Simprints GUID through
    // updateBiometricsAttributeValue(), which reads the active biometrics config from preferences.
    ...
    // Simprints behavior: BiometricsDuplicatesDialogModule lives in another package and needs to
    // build its own instance to resolve duplicate candidates, so this is public.
    ...
    // EyeSeeTea customization - Biometric Verification Persistence
    // Writes the Simprints GUID into the biometrics tracked entity attribute so duplicate
    // review and follow-up search resolve through normal DHIS2 search.
    @Override
    ...
    // Returns the user's capture-scope org units, optionally narrowed to a program, so search
    // biometric identify can derive the Simprints moduleId from the org unit hierarchy.
    @Override
